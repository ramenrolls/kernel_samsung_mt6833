    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		pr_err("cwd return_sensor_id:0x%x\n",*sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {               
				//+bug767771, liudijin.wt, ADD, 2022/07/06, add eeprom bringup code.
				size = imgSensorReadEepromData(&w2sc202csmicrolh_eeprom_data,w2sc202csmicrolh_checksum);
				if((size != w2sc202csmicrolh_eeprom_data.dataLength) || ((w2sc202csmicrolh_eeprom_data.sensorVendorid >> 24)!= read_eeprom(0x0001))) {
					...
					imgSensorSetEepromData(&w2sc202csmicrolh_eeprom_data);
				}
				//-bug767771, liudijin.wt, ADD, 2022/07/06, add eeprom bringup code.
